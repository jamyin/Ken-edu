...
import com.ssic.test.common.mapper.AddressExMapper;
import com.ssic.test.common.mapper.AddressMapper;
import com.ssic.test.common.pojo.Address;
import com.ssic.test.common.pojo.AddressExample;
import com.ssic.test.common.pojo.AddressExample.Criteria;
...
public class AddressDao extends MyBatisBaseDao<Address> {
	...
	public List<Address> findAllBy(Address param, PageHelperDto ph) {
		...
		Criteria criteria = example.createCriteria();
		if (ph != null
				&& !StringUtils.isEmpty(String.valueOf(ph.getBeginRow()))
				&& !StringUtils.isEmpty(String.valueOf(ph.getRows()))) {
			int beginRow = ph.getBeginRow();
			int rows = ph.getRows();
			example.setOrderByClause("create_time desc limit " + beginRow + ","
					+ rows);
		} else {
	...
	public AddressDto validAddressRootExists(AddressDto addressDto) {
		AddressDto dto = exMapper.validAddressRootExists(addressDto.getId(),
				addressDto.getProjId());
		return dto;
	}
	...
	public List<Address> findAddressByTypeAndAddressCode(Integer type,
		...
		if (type == 1) {
			if (addressCode.length() == 2) {
				...
				criteria.andAddressCodeLike(addressCode.substring(0,
						addressCode.length() - 3) + "__");
			}
		} else if (type == 2) {
