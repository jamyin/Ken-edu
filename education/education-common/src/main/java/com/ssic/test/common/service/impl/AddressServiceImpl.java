 ...
 * 
 */
package com.ssic.test.common.service.impl;
...
import com.ssic.test.common.util.MyComparator;
...
public class AddressServiceImpl implements IAddressService {
	...
	public List<AddressDto> treeGrid(AddressDto addressDto) {
		...
		if (dtoList != null && dtoList.size() > 0) {
			for (AddressDto addressResult : dtoList) {
				ProjectDto project = projectDao.findById(addressResult
						.getProjId());
				...
				if (!StringUtils.isEmpty(addressResult.getParentId())) {
					Address addredto = addressDao.findById(addressResult
							.getParentId());
	...
	public List<AddressDto> findChildListByParentId(String id) {
		...
		if (!CollectionUtils.isEmpty(addressDtoList)) {
			result = BeanUtils.createBeanListByTarget(addressDtoList,
					AddressDto.class);
		}
		return result;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ssic.catering.base.service.IAddressService#validDeptRootExists(java.lang.String,
	 *      java.lang.String)
	 */
	@Override
	...
	public void setAddressCode(AddressDto address) {
		...
		if (null != dto) {
			...
			if (dto2 != null) {
				...
				// 当前部门原来的父部门对象下的子部门集合;
				List<Address> dtoLists = addressDao.findChildListByParentId(dto
						.getParentId());
				...
				if (!CollectionUtils.isEmpty(newDtoList)) {
					...
					// 如：原部门下有:0001 0002 0003这3个，现在移动0002,则0001
					// 0003需要变为0001,0002 ;
					// 如
					// 原来父部门编码是00，有3个子部门，现在循环（3-1）次，当前子部门编码为："00"+(01)=0001,"00"+(02)=0002;
					for (int i = 0; i < newDtoList.size(); i++) {
						AddressDto addressDto = newDtoList.get(i);
						String dCode = "";
						if (i < 9) {
							dCode = lastParentDeptCode + ("0" + (i + 1));
						} else {
							dCode = lastParentDeptCode + ((i + 1));
						}
						addressDto.setAddressCode(dCode);
						addressDto.setParentCode(lastParentDeptCode);
		...
		else { // 如果当前节点是3级节点第2条：如00（1级）01（2级）02（当前），则父节点是0001;如果父节点下有3个子节点，
				// 则第一个是000101，第二个是000102,第三个是000103;当前添加的则应该是父节点的第4个节点:000104;
			String parentDeptCode = addressDao.findById(address.getPid())
					.getAddressCode();
			String resultDeptCode = null;
			// 查找父节点下的子节点;
			List<Address> dtoList = addressDao.findChildListByParentId(address
					.getPid());
			// List<DeptDto> dtoList2 =
			// deptDao.findChildListByParentId(dept.getId());
			if (!CollectionUtils.isEmpty(dtoList)) { // 当前的code;
				String localDeptCode = "";
				if (dtoList.size() < 9) {
					localDeptCode = "0" + String.valueOf(dtoList.size() + 1);
				} else {
					localDeptCode = String.valueOf(dtoList.size() + 1);
				}
				resultDeptCode = parentDeptCode + localDeptCode;
				address.setAddressCode(resultDeptCode);
				address.setParentCode(parentDeptCode);
			} else {
	...
	public void updateChildAddress(String addressId, String parentAddressCode) {
		...
		if (!CollectionUtils.isEmpty(childs)) {
			for (int j = 0; j < childs.size(); j++) {
				String lastParentCode="";
				if (j < 9) {
					 lastParentCode = parentAddressCode + ("0" + (j + 1));
				}else{
					 lastParentCode = parentAddressCode + ( (j + 1));
				}
	...
	public String getRootDeptCode() {
		...
		if (!CollectionUtils.isEmpty(dtoList)) {// 如果存在根节点的deptCode,如：08；则当前应给部门的部门编码为09
			...
			if (Integer.valueOf(lastAddressCode) < 9) {
				...
				defaultCode = String
						.valueOf((Integer.valueOf(lastAddressCode) + 1));
			}
		}
		return defaultCode;
	}
	...
	@Override
	public List<Address> findAddressByTypeAndAddressCode(Integer type,
			String addressCode, String projId) {
