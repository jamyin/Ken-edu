...
public class TaskController {
	 ...
	 * @Description: 发送通知(任务)
	 * @author Ken Yin  
	 * @date 2016年5月23日 下午6:02:44
	 * @return Response<String>    返回类型
	 */
	public Response<EduTaskDto> sendTask(EduTaskDto eduTaskDto) {
		...
		EduTaskDto dto = taskService.addTask(eduTaskDto);

		List<EduTaskReceiveDto> receiveDtolist = new ArrayList<EduTaskReceiveDto>();
		if(dto == null){
			result.setStatus(DataStatus.HTTP_FAILE);
			result.setMessage("新建任务失败");
			return result;
		}
		if(StringUtils.isNotEmpty(eduTaskDto.getReceiveId())){
			String receive[] = eduTaskDto.getReceiveId().split(","); 
			EduTaskReceiveDto receiveDto = new EduTaskReceiveDto();
			for(int i = 0;i < receive.length; i++){
				receiveDto.setReceiveId(receive[i]);
				receiveDto.setId(UUIDGenerator.getUUID());
				receiveDto.setTaskId(dto.getId());
		...
		int flag = taskService.addReceiveTask(receiveDtolist);
		if(flag > 0){
			result.setStatus(DataStatus.HTTP_SUCCESS);
